...
function add(text,metadata={}){ const store=load(); if(store.some(s=>s.text===text))return; store.push({text,vec:buildVec(tokenize(text)),metadata,ts:Date.now()}); save(store.slice(-500)); }

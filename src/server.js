...
app.post('/auth/login',(req,res)=>{const{username,password}=req.body;if(!username||!password)return res.status(400).json({error:'Missing fields'});const token=auth.login(username,password);if(!token)return res.status(401).json({error:'Invalid credentials'});res.json({token,username});});
...
app.post('/chat', auth.authMiddleware, async(req,res)=>{ const {message,session_id='default',voice=false}=req.body; const sessionKey = req.user.username + '_' + (session_id||'default'); if (!message) return res.status(400).json({error:'message required'}); try { const r=await chat(message,sessionKey); if (voice&&r.reply) r.audio_b64=await textToSpeech(r.reply); res.json(r); } catch(e){ res.status(500).json({error:e.message}); } });

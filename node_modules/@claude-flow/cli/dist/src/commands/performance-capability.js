...
function resolveApiKey() {
    ...
    throw new Error('ANTHROPIC_API_KEY not found. Set the env var or store it as a gcloud secret named ANTHROPIC_API_KEY (e.g. `echo -n "$KEY" | gcloud secrets versions add ANTHROPIC_API_KEY --data-file=-`).');
}
function loadTaskFile(custom) {
    if (custom) {
        const resolved = path.resolve(custom);
        if (!fs.existsSync(resolved))
            throw new Error(`questions file not found: ${resolved}`);
        return JSON.parse(fs.readFileSync(resolved, 'utf-8'));
    }
    return BUILTIN_CAPABILITY_TASKS;
}
...
async function runOne(task, model, apiKey, defaultMaxTokens, timeoutMs) {
    const maxTokens = task.maxTokens ?? defaultMaxTokens;
    const start = performance.now();
    try {
        const { text, inputTokens, outputTokens } = await callAnthropic(apiKey, model, buildPrompt(task), maxTokens, timeoutMs);
...
function summarizeModel(results) {
...
const capabilityCommand = {
    ...
    action: async (ctx) => {
        const modelsFlag = ctx.flags.models;
        const singleModel = ctx.flags.model || 'claude-haiku-4-5';
        const models = modelsFlag
            ? modelsFlag.split(',').map((m) => m.trim()).filter(Boolean)
            : [singleModel];
        const customPath = ctx.flags.questions;
        ...
        try {
            ...
            output.writeln(output.error(err.message));
            return { success: false, message: err.message, exitCode: 1 };
        }
        let file;
        try {
...
//# sourceMappingURL=performance-capability.js.map
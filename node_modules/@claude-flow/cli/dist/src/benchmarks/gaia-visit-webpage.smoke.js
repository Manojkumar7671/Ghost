...
let _visitWebpage;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let _visitWebpageTestHooks;
let _createVisitWebpageTool;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
let _extractTextViaRegex;
// ---------------------------------------------------------------------------
// Fixture HTML
// ---------------------------------------------------------------------------
...
const TESTS = [
    ...
    // -------------------------------------------------------------------------
    {
        ...
        async run() {
            _visitWebpageTestHooks.fetchHtml = async (_url) => ({
    ...
    // -------------------------------------------------------------------------
    {
        ...
        async run() {
            ...
            assert(result.content.includes('Babbage') || result.content.includes('Difference Engine'), `Expected article text, got: ${result.content.slice(0, 200)}`);
        },
    },
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    {
        ...
        async run() {
            ...
            try {
                ...
                const msg = String(err);
                assert(msg.includes('visit_webpage') && (msg.includes('URL') || msg.includes('url')), `Expected validation error, got: ${msg}`);
            }
        },
    },
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    {
        ...
        async run() {
            ...
            assert(result.sources_consulted.includes('https://example.com/original'), 'sources_consulted missing original URL');
            assert(result.sources_consulted.includes('https://example.com/redirected-page'), 'sources_consulted missing final URL');
        },
    },
];
// ---------------------------------------------------------------------------
...
async function main() {
    ...
    if (typeof _visitWebpage !== 'function') {
        console.error('ERROR: visitWebpage not exported from gaia-tools/index.\n' +
...
export {};
//# sourceMappingURL=gaia-visit-webpage.smoke.js.map
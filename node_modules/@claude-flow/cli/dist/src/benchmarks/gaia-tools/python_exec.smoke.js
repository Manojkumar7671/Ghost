...
const tests = [
    ...
    runTest('Test 2: large computation — sum(range(1000000))', async () => {
        ...
        assert(out.includes('499999500000'), `Expected "499999500000", got:\n${out}`);
        assert(out.includes('exit_code: 0'), `Expected exit_code 0, got:\n${out}`);
    }),
    ...
    runTest('Test 4: timeout — infinite loop with 2s limit', async () => {
        ...
        assert(elapsed < 6_000, `Timeout should have fired within ~2s, took ${elapsed}ms`);
        const hasTimedOut = out.includes('timed out') ||
    ...
    runTest('Test 5: syntax error — exit_code != 0, stderr contains SyntaxError', async () => {
        ...
        assert(out.includes('SyntaxError'), `Expected SyntaxError in output, got:\n${out}`);
        const hasNonZeroExit = !out.includes('exit_code: 0') || out.includes('stderr:');
        assert(hasNonZeroExit, `Expected non-zero exit code or stderr for syntax error, got:\n${out}`);
    }),
];
// ---------------------------------------------------------------------------
// Runner
...
//# sourceMappingURL=python_exec.smoke.js.map
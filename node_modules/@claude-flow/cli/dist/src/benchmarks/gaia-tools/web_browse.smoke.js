...
async function testWikipedia() {
    ...
    assert(out.toLowerCase().includes('mercedes') || out.toLowerCase().includes('sosa'), `Expected page text to contain "Mercedes" or "Sosa", got:\n${out.slice(0, 500)}`);
    assert(out.includes('final_url:'), `Expected final_url in output, got:\n${out.slice(0, 200)}`);
}
// Test 2: Bad URL with short timeout — should return gracefully, not throw
async function testBadUrlTimeout() {
    const out = await tool.execute({
...
async function testScreenshotMode() {
    const out = await tool.execute({
    ...
    if (contentIdx !== -1) {
        ...
        // Base64 charset only
        assert(/^[A-Za-z0-9+/=]+$/.test(b64.slice(0, 100)), `Expected base64-encoded output, got:\n${b64.slice(0, 100)}`);
    }
}
// Test 4: HTML extract mode
async function testHtmlExtract() {
    const out = await tool.execute({
        url: 'https://en.wikipedia.org/wiki/Main_Page',
        extract: 'html',
        timeout_seconds: 30,
    ...
    assert(out.toLowerCase().includes('<html') || out.includes('<!DOCTYPE'), `Expected HTML markup in output, got:\n${out.slice(0, 300)}`);
}
// ---------------------------------------------------------------------------
// Runner
// ---------------------------------------------------------------------------
...
//# sourceMappingURL=web_browse.smoke.js.map
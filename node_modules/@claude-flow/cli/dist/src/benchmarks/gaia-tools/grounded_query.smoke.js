...
import { GroundedQueryTool, groundedQueryTestHooks, resolveGoogleAIApiKey, parseGeminiResponse, createGroundedQueryTool, } from './grounded_query.js';
// ---------------------------------------------------------------------------
...
async function runSuite() {
    ...
    await test('resolveGoogleAIApiKey: throws with guidance when no key available', async () => {
        ...
        try {
            await assert.rejects(() => tool.execute({ query: 'test missing key' }), /GOOGLE_AI_API_KEY/);
        }
        finally {
            clearHooks();
        }
    ...
    await test('parseGeminiResponse: cost_usd calculated from token counts', async () => {
        ...
        assert.ok(result.cost_usd >= expectedMin && result.cost_usd <= expectedMax, `cost_usd=${result.cost_usd} not in [${expectedMin}, ${expectedMax}]`);
    });
    // -------------------------------------------------------------------------
    ...
    await test('GroundedQueryTool.execute: HTTP error surfaces in thrown Error', async () => {
        ...
        const tool = new GroundedQueryTool();
        try {
            await assert.rejects(() => tool.execute({ query: 'test error' }), /Gemini HTTP 400/);
        }
        finally {
            clearHooks();
        }
...
//# sourceMappingURL=grounded_query.smoke.js.map
...
async function loadPlaywright() {
    try {
        ...
        const pw = await import(/* @vite-ignore */ pwModuleName);
        return { ok: true, chromium: pw.chromium };
    }
    catch {
        return {
            ok: false,
            reason: 'Playwright is not installed.  Install it with:\n' +
                '  npm install playwright\n' +
                '  npx playwright install chromium\n' +
...
// Core browse logic
// ---------------------------------------------------------------------------
async function browseUrl(input) {
    const extract = input.extract ?? 'text';
    const timeoutMs = Math.min(Math.max(1_000, Math.round((input.timeout_seconds ?? DEFAULT_TIMEOUT_MS / 1000) * 1000)), 120_000);
...
// ---------------------------------------------------------------------------
function formatBrowseResult(result, extract) {
    const lines = [];
...
export class WebBrowseTool {
    ...
    definition = {
        name: 'web_browse',
        description: 'Open a URL in a headless Chromium browser and extract page content. ' +
        ...
        input_schema: {
            ...
            properties: {
                ...
                extract: {
                    type: 'string',
                    description: 'What to extract: "text" (default, visible text), ' +
    ...
    async execute(input) {
        ...
        if (!url)
            throw new Error('web_browse: `url` input is required and must be non-empty.');
        const rawExtract = String(input['extract'] ?? 'text').toLowerCase();
        const extract = rawExtract === 'html' ? 'html' : rawExtract === 'screenshot' ? 'screenshot' : 'text';
        const browseInput = {
            url,
            wait_for_selector: input['wait_for_selector'] != null ? String(input['wait_for_selector']) : undefined,
            extract,
            timeout_seconds: input['timeout_seconds'] != null ? Number(input['timeout_seconds']) : undefined,
        };
        const result = await browseUrl(browseInput);
        return formatBrowseResult(result, extract);
    }
}
export function createWebBrowseTool(_opts) {
    return new WebBrowseTool();
}
//# sourceMappingURL=web_browse.js.map
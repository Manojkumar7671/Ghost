...
import { WebSearchTool, webSearchTestHooks, resolveGoogleCustomSearchCredentials, } from './web_search.js';
// ---------------------------------------------------------------------------
...
// Stub results
const GOOGLE_RESULT = { title: 'Google Result', url: 'https://g.com/r', snippet: 'from google' };
const WIKI_RESULT = { title: 'Wiki Result', url: 'https://en.wikipedia.org/wiki/Test', snippet: 'from wikipedia' };
const DDG_HTML = `
...
async function runSuite() {
    ...
    await test('resolveGoogleCustomSearchCredentials: returns creds from env vars', async () => {
        ...
        try {
            ...
            if (savedKey === undefined)
                ...
                process.env['GOOGLE_CUSTOM_SEARCH_API_KEY'] = savedKey;
            if (savedCx === undefined)
                delete process.env['GOOGLE_CUSTOM_SEARCH_CX'];
            else
                process.env['GOOGLE_CUSTOM_SEARCH_CX'] = savedCx;
        }
    });
    // -------------------------------------------------------------------------
    ...
    await test('WebSearchTool.execute: falls back to DDG when Google + Wikipedia both fail', async () => {
        ...
        webSearchTestHooks.ddgFetch = async () => '<a class="result__a" href="https://example.com/ddg">DDG Result</a>' +
            '<a class="result__snippet">DDG snippet</a>';
        const tool = new WebSearchTool();
        try {
    ...
    await test('WebSearchTool.execute: rejects empty query', async () => {
        const tool = new WebSearchTool();
        await assert.rejects(() => tool.execute({ query: '' }), /query.*required/);
    });
    // -------------------------------------------------------------------------
...
//# sourceMappingURL=web_search.smoke.js.map
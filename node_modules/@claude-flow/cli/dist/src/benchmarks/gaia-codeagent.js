...
function getRunnerPath() {
    ...
    try {
        ...
        const srcBenchmarks = distBenchmarks.replace(path.join('dist', 'src', 'benchmarks'), path.join('src', 'benchmarks'));
 ...
 */
function buildCodeAgentSystemPrompt() {
    const toolDocs = [
...
// Planning checkpoint
// ---------------------------------------------------------------------------
function buildCodeAgentPlanningCheckpoint(turn, maxTurns) {
    return (`[PLANNING CHECKPOINT — step ${turn}/${maxTurns}]\n` +
 ...
 */
function executeAgentCodeStep(code, attachmentPath, stepTimeoutMs, apiKey, toolCallsByName) {
    ...
    try {
        ...
        const stderrClean = stderr
            ? stderr.split('\n').filter(l => !l.includes('UserWarning') && !l.includes('DeprecationWarning') && l.trim()).slice(0, 5).join('\n')
            : '';
 ...
 */
export function runCodeAgentStep(code, attachmentPath, timeoutMs = DEFAULT_PER_STEP_TIMEOUT_MS, apiKey = '') {
...
// ---------------------------------------------------------------------------
async function callAnthropicTextOnly(apiKey, model, systemPrompt, messages, maxTokens, timeoutMs) {
 ...
 */
export async function runGaiaCodeAgent(question, options = {}) {
    const { model = DEFAULT_MODEL, maxTurns = DEFAULT_MAX_TURNS, maxTokensPerTurn = DEFAULT_MAX_TOKENS_PER_TURN, perTurnTimeoutMs = DEFAULT_PER_TURN_TIMEOUT_MS, perStepTimeoutMs = DEFAULT_PER_STEP_TIMEOUT_MS, planningInterval = DEFAULT_PLANNING_INTERVAL, apiKey: suppliedKey, verbose = false, } = options;
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
        ...
        let resp;
        try {
            resp = await callAnthropicTextOnly(apiKey, model, systemPrompt, messages, maxTokensPerTurn, perTurnTimeoutMs);
        }
        catch (err) {
            return {
        ...
        const { output, finalAnswer: execFinalAnswer } = executeAgentCodeStep(codeBlock, attachmentPath, perStepTimeoutMs, apiKey, toolCallsByName);
        if (verbose) {
        ...
        const shouldReplan = planningInterval > 0 && turns > 0 && turns % planningInterval === 0;
 ...
 */
function buildGaiaQuestionMessage(question) {
    const gaiaPreamble = [
...
export async function runCodeAgentSmokeTest(opts = {}) {
    ...
    for (const question of SMOKE_FIXTURE) {
        ...
        const correct = result.finalAnswer !== null &&
        ...
        if (verbose) {
            ...
            console.log(`       Turns: ${result.turns} | Replans: ${result.replanCount} ` +
    ...
    const estCost = (totalInputTokens / 1_000_000) * SONNET_INPUT_COST_PER_M +
...
//# sourceMappingURL=gaia-codeagent.js.map
...
function loadPilotQuestions() {
    ...
    for (const id of PILOT_IDS) {
        const q = byId.get(id);
        if (!q)
            throw new Error(`Pilot question ${id} not found in cache`);
...
async function runPilot() {
    ...
    for (let i = 0; i < questions.length; i++) {
        ...
        console.log(`  Status: ${status} | turns=${result.turns} | ` +
    ...
    const outPath = path.join(process.cwd(), 'docs/benchmarks/runs/gaia-l1-phase2-gemini-pilot.json');
...
//# sourceMappingURL=gaia-gemini-pilot.js.map
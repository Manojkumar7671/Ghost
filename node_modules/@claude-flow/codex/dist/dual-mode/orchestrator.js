...
export class DualModeOrchestrator extends EventEmitter {
    ...
    async initializeSharedMemory(taskContext) {
        ...
        await this.runCommand('npx', ['ruflo@alpha', 'memory', 'init', '--force'], projectPath);
        // Store task context
    ...
    async executeHeadless(config) {
        ...
        if (config.platform === 'claude') {
            ...
            // codex-cli 0.130.0 exec mode. Default sandbox is workspace-write
            // (matches pre-alpha.11 behavior). `codexFlags` lets callers opt
            // into 0.130.0 features (read-only sandbox, JSONL output, schema
            // constraints, output-last-message). See #6 codex review pass.
            const flags = config.codexFlags ?? {};
            const sandbox = flags.sandbox ?? 'workspace-write';
            args = ['exec', '--sandbox', sandbox, '--skip-git-repo-check'];
            if (flags.askForApproval) {
                args.push('--ask-for-approval', flags.askForApproval);
            }
            if (flags.json) {
                args.push('--json');
            }
            if (flags.outputLastMessage) {
                args.push('--output-last-message', flags.outputLastMessage);
            }
            if (flags.outputSchema) {
                args.push('--output-schema', flags.outputSchema);
            }
            if (config.model) {
        ...
        return new Promise((resolve, reject) => {
            ...
            proc.on('close', (code) => {
                ...
                // Non-zero exit is a failure regardless of stdout content. Before
                // this fix, the orchestrator treated any non-empty stdout as
                // success and silently dropped the exit code, so a worker that
                // printed a partial result then crashed looked indistinguishable
                // from a clean run. Surfaced by codex review (2026-05-17 pass).
                if (code === 0) {
                    resolve(output);
                }
                else {
                    // Preserve BOTH stdout and stderr in the error so callers can
                    // debug without losing the partial output.
                    const detail = errorOutput || output || '(no output captured)';
...
Remember: Other agents depend on your results in shared memory. Be concise and store actionable outputs.`;
    ...
    async runCollaboration(workers, taskContext) {
        ...
        // Execute each level — workers within a level can run in parallel,
        // but capped at `maxConcurrent`. Before this fix the entire level
        // was launched at once via `Promise.all`, so `maxConcurrent: 1`
        // (and every value below the level size) was silently ignored.
        // Surfaced by codex review (2026-05-17 pass).
        const concurrency = Math.max(1, this.config.maxConcurrent);
        for (const level of levels) {
            // Each "lane" pulls workers from a shared queue. With N lanes,
            // at most N workers run at once; lanes settle when the queue is
            // empty, so the level finishes when every worker has run.
            const queue = [...level];
            const runLane = async () => {
                while (queue.length > 0) {
                    ...
                    try {
                        ...
                        errors.push(`${worker.id}: ${err instanceof Error ? err.message : String(err)}`);
                    }
                }
            };
            const lanes = Array.from({ length: Math.min(concurrency, level.length) }, () => runLane());
    ...
    buildDependencyLevels(workers) {
        ...
        while (placed.size < workers.length) {
            ...
            if (level.length === 0 && placed.size < workers.length) {
                // Circular dependency. Before alpha.11 we silently enqueued the
                // remaining workers into the level, expecting them to "work
                // themselves out" — but `waitForDependencies` blocks each one
                // until its deps complete, and they never will, so the whole
                // batch just times out (default 5 min per worker). Throw
                // immediately with the cycle members named so the operator
                // sees the actual problem instead of a wall of timeout errors.
                // Surfaced by codex review (#4, 2026-05-17 pass).
                const unplaced = workers.filter(w => !placed.has(w.id)).map(w => w.id);
                throw new Error(`DualModeOrchestrator: circular dependency detected — unable to place workers ` +
                    `[${unplaced.join(', ')}]. Check the dependsOn graph for cycles.`);
            }
    ...
    async collectSharedMemory() {
        const { projectPath, sharedNamespace } = this.config;
        try {
            const output = await this.runCommand('npx', ['ruflo@alpha', 'memory', 'list', '--namespace', sharedNamespace, '--format', 'json'], projectPath);

...
export async function migrateFromClaudeCode(options) {
    ...
    void preserveComments; // Reserved for future per-section comment retention.
    try {
        // Real migration: read source, parse, generate outputs, write to disk.
        // Before this rewrite, this function returned a fabricated success
        // shape without reading sourcePath or writing anything — the CLI
        // displayed "Migrating to Codex..." but nothing actually migrated.
        // Surfaced by codex review (2026-05-17 pass).
        const content = await fs.readFile(sourcePath, 'utf-8');
        const migrated = await performFullMigration(content);
        // Write AGENTS.md to the target. Ensure the directory exists so
        // first-time runs against a fresh dir don't fail.
        const agentsMdPath = path.join(targetPath, 'AGENTS.md');
        await fs.mkdir(targetPath, { recursive: true });
        await fs.writeFile(agentsMdPath, migrated.agentsMd, 'utf-8');
        // Write config.toml under .agents/ (matches the initializer layout).
        const configDir = path.join(targetPath, '.agents');
        await fs.mkdir(configDir, { recursive: true });
        const configTomlPath = path.join(configDir, 'config.toml');
        await fs.writeFile(configTomlPath, migrated.configToml, 'utf-8');
        // Generate per-skill markdown files when asked. Each goes under
        // .agents/skills/<skill>/SKILL.md so codex picks them up.
        const skillsCreated = [];
        if (generateSkills) {
            const skillsDir = path.join(configDir, 'skills');
            await fs.mkdir(skillsDir, { recursive: true });
            // Use the generated skill list from the parser, not a fabricated one.
            for (const skillName of migrated.skillsToCreate) {
                const skillDir = path.join(skillsDir, skillName);
                await fs.mkdir(skillDir, { recursive: true });
                const skillContent = `---\nname: ${skillName}\ndescription: Migrated from Claude Code\n---\n\n# ${skillName}\n\n(Generated by @claude-flow/codex migration. Review and customize.)\n`;

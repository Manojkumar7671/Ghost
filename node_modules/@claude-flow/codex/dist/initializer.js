...
export class CodexInitializer {
    ...
    async createDirectoryStructure() {
        ...
        // ensureDir is idempotent and these directories don't depend on
        // each other (each call already creates parents). Run in parallel
        // — saves N-1 disk round-trips on init. Surfaced by codex review
        // #8 (2026-05-17 pass).
        await Promise.all(dirs.map(dir => fs.ensureDir(path.join(this.projectPath, dir))));
    }
    /**
    ...
    async copyBundledSkills() {
        const copied = [];
        const warnings = [];
        // Check if bundled skills directory exists. When the package is
        // installed from npm (not the monorepo), `bundledSkillsPath`
        // resolves to a path under node_modules that doesn't exist —
        // `files` in package.json only ships `dist`. Fall back to
        // *generating* the skill markdown via the in-repo generator so
        // users still get a working .agents/skills/ tree.
        // Surfaced by codex review (2026-05-17 pass).
        if (!await fs.pathExists(this.bundledSkillsPath)) {
            const generated = await this.generateBundledSkillsFallback();
            copied.push(...generated.copied);
    ...
    /**
     * Fallback when the bundled skills directory isn't shipped (npm install
     * case — package.json `files` only carries `dist`). Generates SKILL.md
     * for every skill in the active template's list using the in-package
     * `generateBuiltInSkill` generator, so users still get a populated
     * `.agents/skills/` tree even when no on-disk source dir exists.
     *
     * Returns the list of skill names written + any per-skill warnings.
     * Fixed in alpha.10 (codex review 2026-05-17).
     */
    async generateBundledSkillsFallback() {
        const copied = [];
        const warnings = [];
        const destSkillsDir = path.join(this.projectPath, '.agents', 'skills');
        await fs.ensureDir(destSkillsDir);
        const includeAll = this.template === 'full' || this.template === 'enterprise';
        const skillSet = includeAll ? ALL_AVAILABLE_SKILLS : this.skills;
        // Generate each skill in parallel. Each skill is independent — its
        // destination directory and file set don't collide with other
        // skills. Per-skill errors are still surfaced as warnings, not
        // aborts. #8 codex review optimization.
        const results = await Promise.all(skillSet.map(async (skillName) => {
            const destDir = path.join(destSkillsDir, skillName);
            const destFile = path.join(destDir, 'SKILL.md');
            if (!this.force && await fs.pathExists(destFile)) {
                return { skillName, kind: 'skipped', message: `Skill ${skillName} already exists - skipped` };
            }
            try {
                // generateBuiltInSkill is async and returns
                //   { skillMd, scripts, references }
                // — write each piece into the skill dir. SKILL.md is the
                // entry point codex picks up; scripts/* and references/* go
                // alongside it. Throws when skillName isn't in the built-in
                // catalog, which we treat as a warning (not a fatal error).
                const bundle = await generateBuiltInSkill(skillName);
                await fs.ensureDir(destDir);
                // Per-skill file writes can also run in parallel — they all
                // target the same destDir but distinct filenames.
                const writes = [
                    fs.writeFile(destFile, bundle.skillMd, 'utf-8'),
                ];
                for (const [scriptName, scriptContent] of Object.entries(bundle.scripts || {})) {
                    writes.push(fs.writeFile(path.join(destDir, scriptName), scriptContent, 'utf-8'));
                }
                for (const [refName, refContent] of Object.entries(bundle.references || {})) {
                    writes.push(fs.writeFile(path.join(destDir, refName), refContent, 'utf-8'));
                }
                await Promise.all(writes);
                return { skillName, kind: 'copied' };
            }
            catch (err) {
                // generateBuiltInSkill throws for unknown skill names — that's
                // expected for any skill not in its built-in catalog. Surface
                // as a warning, not an error: the rest of the scaffold still
                // succeeds.
                const errorMessage = err instanceof Error ? err.message : String(err);
                return { skillName, kind: 'failed', message: `Generated-skill fallback failed for ${skillName}: ${errorMessage}` };
            }
        }));
        for (const r of results) {
            if (r.kind === 'copied')
                copied.push(r.skillName);
     ...
     *
     * Codex 0.130.0 integration improvements (alpha.12 audit):
     *  - Use `codex mcp get <name>` for a precise per-server existence
     *    check instead of grepping `codex mcp list`. The list-grep
     *    matched substrings (e.g. "ruflo" anywhere in any other server's
     *    metadata) and missed the case where the *same* MCP server is
     *    already registered under the legacy `claude-flow` name.
     *  - Skip registration when *either* `ruflo` OR `claude-flow` is
     *    already present — both names point at the same MCP server,
     *    and double-registering breaks tool discoverability on the
     *    codex side ("which ruflo_* tool came from which registration?").
     */
    async registerMCPServer() {
        try {
            ...
            // Per-name existence check via `codex mcp get`. The command exits
            // 0 if the server exists and non-zero otherwise — simpler and
            // more reliable than grepping the table output of `mcp list`.
            const exists = (name) => {
                try {
                    execSync(`codex mcp get ${name}`, { stdio: 'pipe' });
                    return true;
                }
                catch {
                    return false;
            ...
            if (exists('ruflo') || exists('claude-flow')) {
